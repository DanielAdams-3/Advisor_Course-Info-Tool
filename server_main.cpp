...
int main() {
    ...
    string filename =  "cirt_data.csv";
    bool load_status = dict.load(filename); //dict.load calls BuildTrie(), returns false if trie is empty
    ...
    if (load_status == false) {
        std::cerr << "Warning: could not load cirt_data.csv; "
